Snap.plugin(function (Snap, Element, Paper, global) {
  var partSystem = function (options, parent) {
    //Setup presets
    var presets = {
        maxParticles: 150,
        size: 18,
    ...
    this.parent = parent;
    this.parentNode = parent.el('g', {class: 'particleSystem'});
  };
  ...
  partSystem.prototype.addParticle = function () {
      ...
      if (this.recycledParticles.length) {
        ...
        this.parentNode.append(particle.element);
      } else {
  ...
  partSystem.prototype.initParticle = function (particle) {
      ...
      if (particle.size < 0.1) {
        console.log("Invisible", particle);
      }
  ...
  partSystem.prototype.update = function () {
      ...
      while (this.particleIndex < this.particleCount) {
          ...
          if (currentParticle.timeToLive > 0) {
              ...
              currentParticle.element.remove();
              this.particleCount--;
          }
          var rect = {};
  ...
  partSystem.prototype.updateDOM = function (context) {
      for (var i = 0, j = this.particleCount; i < j; i++) {
          ...
          particle.element.attr({
            height: size,
            width: size,
            x: x,
            y: y,
            fill: particle.drawColour
          });
      }
  };
  partSystem.prototype.particle = function (that) {
      this.position = that.vectorHelpers.create(0, 0);
      this.direction = that.vectorHelpers.create(0, 0);
      this.size = 0;
      this.timeToLive = 0;
      this.colour = [];
      this.drawColour = "";
      this.deltaColour = [];
      this.rotation = 0;
      this.element = that.parentNode.rect(0,0,0,0);
  };
  partSystem.prototype.vectorHelpers = {
      create: function (x, y) {
          return {
              x: x,
              y: y
  ...
  Paper.prototype.ParticleSystem = partSystem;
});
